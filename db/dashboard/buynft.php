...
<?php
// Fetch the relevant data from the 'buy' table
$sql = "SELECT nft_amount, author_name, nft_picture FROM buy WHERE status = 'pending' ORDER BY id DESC LIMIT 1";
...
if ($result->num_rows > 0) {
    ...
    $nft_picture = $row['nft_picture'];
} else {
    // Handle if no pending record is found
    echo "No pending record found.";
    exit();
}
...
<div class="grid grid-cols-12">
    <div class="col-span-12 mx-auto mt-8">
        <div class="card p-8" style="padding: 30px;">
    <div class="flex items-center space-x-6">
      <div class="avatar h-20 w-20">
        <img class="mask is-squircle" src="<?php echo $nft_picture; ?>" alt="avatar">
      </div>
      <div>
   ...
   <div class="mt-8">
    <label for="amount" class="block text-sm font-medium text-slate-700 dark:text-navy-100">Amount</label>
...
">
        <input style="background-color: #02012b;" type="text" id="amount" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-navy-700 dark:border-navy-600 dark:placeholder-navy-400 dark:text-navy-100" value="<?php echo $nft_amount; ?>" readonly="">
        <p class="ml-2">ETH</p>
    </div>
...
<hr>
<div class="mt-4">
    <label for="total" class="block text-sm font-medium text-slate-700 dark:text-navy-100">Total</label>
    <div class="flex items-center">
        <input type="text" id="total" class="mt-1 block w-full rounded-md border-slate-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm dark:bg-navy-700 dark:border-navy-600 dark:placeholder-navy-400 dark:text-navy-100" value="<?php echo $nft_amount; ?>" readonly>
        <p class="ml-2">ETH</p>
    </div>
</div>

    <div class="mt-8 flex justify-end space-x-2">
     <button id="confirmButton" class="btn space-x-2 rounded-full border border-slate-300 font-medium text-slate-700 hover:bg-slate-200 focus:bg-slate-200 active:bg-slate-200/80 dark:border-navy-450 dark:text-navy-100 dark:hover:bg-navy-450/20 dark:focus:bg-navy-450/20 dark:active:bg-navy-450/25" style="
...
<style>
    .alert-message {
        ...
        position: fixed; /* Position the alert message fixed */
        top: 50%; /* Position the alert message at the top center */
        left: 50%; /* Position the alert message at the left center */
        transform: translate(-50%, -50%); /* Center the alert message */
        z-index: 1000; /* Make sure the alert message is on top of everything */
    }

    .hidden {
        display: none; /* Hide the alert message by default */
    }

    .show {
        display: block; /* Show the alert message when the .show class is added */
    }
</style>

<script>
    document.getElementById('confirmButton').addEventListener('click', function() {
        var amount = parseFloat(<?php echo json_encode($nft_amount); ?>);
        var balance = parseFloat(<?php echo json_encode($_SESSION['balance']); ?>);
        ...
        if (balance > 0) {
            alertMessage.innerHTML = 'Art purchase processing for approval. Kindly be patient.';
            alertMessage.style.backgroundColor = '#4CAF50'; // Green background color
            alertMessage.classList.add('show');
            setTimeout(function() {
                window.location.href = 'home.php'; // Redirect to home.php after a delay
            }, 3000); // 3 seconds delay before redirecting
        } else {
            alertMessage.innerHTML = 'Insufficient funds. Your balance is zero.';
            alertMessage.style.backgroundColor = '#f44336'; // Red background color
            alertMessage.classList.add('show');
            setTimeout(function() {
                window.location.href = 'marketplace.php'; // Redirect to marketplace.php after a delay
            }, 3000); // 3 seconds delay before redirecting
        }
    });
</script>
...
<!-- Add this div element to display the alert messages -->
<!-- Add this div element to display the alert messages -->
