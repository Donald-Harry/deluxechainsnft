...
<html lang="en">
  <head>
    ...
    <title>Deluxeartnft - Marketplace</title>
    ...
    <div id="root" class="min-h-100vh flex grow bg-slate-50 dark:bg-navy-900" x-cloak="">
      ...
      <main class="main-content w-full px-[var(--margin-x)] pb-8">
                     ...
                     <small class="text-small">
                        <a href="#list" class="btn space-x-2 bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
            ...
            <div class="flex">
               <div class="swiper mx-0 mt-4 px-[var(--margin-x)] transition-all duration-[.25s] swiper-initialized swiper-horizontal swiper-backface-hidden" x-init="$nextTick(()=>new Swiper($el,{  slidesPerView: 'auto', spaceBetween: 18}))">
                  <div class="swiper-wrapper" id="swiper-wrapper-1232bb1de82210c6f" aria-live="polite">
                                       </div>
              ...
              if (isset ($_SESSION['card_alert'])) {
          echo ($_SESSION['card_alert']);
          unset ($_SESSION['card_alert']);
        }


            ?>
            <div class="flex items-center space-x-4 py-2 lg:py-6">
               ...
               <ul class="hidden flex-wrap items-center space-x-2 sm:flex">
                  <li>You can upload your own curated NTFs here</li>
               </ul>
            </div>  
             <?php 

              if (isset ($_SESSION['billing_alert'])) {
          echo ($_SESSION['billing_alert']);
          unset ($_SESSION['billing_alert']);
        }
            ...
            <?php 
              ...
              if (isset ($_SESSION['token_alert'])) {
          echo ($_SESSION['token_alert']);
          unset ($_SESSION['token_alert']);
        }
            ...
            ?>
               ...
               <div class="col-span-12 sm:col-span-8">
                  <div class="card p-4 sm:p-5">
                     <p class="text-base font-medium text-slate-700 dark:text-navy-100">
                        Complete the token upload form
                     </p>
                     <form action="../controller/coin-manager.php" method="post" enctype="multipart/form-data">
                        <div class="mt-4 space-y-4">
                           <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                            <label class="block sm:col-span-4">
                        <span>Category</span>
                  <select name="category"  x-init="$el._tom = new Tom($el,{create: true,sortField: {field: 'text'}})" >
                        <option style="font-weight:bold;" value="Art">Art</option>
                       <option style="font-weight:bold;" value="Games">Games</option>
                         <option style="font-weight:bold;" value="Photography">Photography</option>
                       </select>
        ...
        <span>Input Product Name:</span>
        <input name="product_name" x-init="$el._tom = new Tom($el,{create:true,plugins: ['caret_position','input_autogrow']})"  type="text" tabindex="-1" required>
    </label>
    <div class="ts-wrapper mt-1.5 w-full multi plugin-caret_position plugin-input_autogrow required invalid has-options">
        <div class="ts-dropdown multi plugin-caret_position plugin-input_autogrow" style="display: none;">
            <div role="listbox" tabindex="-1" class="ts-dropdown-content" id="tomselect-2-ts-dropdown"></div>
        </div>
        <span style="position: absolute; top: -99999px; left: -99999px; width: auto; padding: 0px; white-space: pre;"></span>
    </div>
                           </div>
                           ...
                           <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                              <label class="block">
                                 ...
                                 <select  name="collection_name" x-init="$el._tom = new Tom($el,{create: true,sortField: {field: 'text',direction: 'asc'}})" id="tomselect-3" tabindex="-1">
                                                                           
                                                                           <option  style="font-weight:bold;" value="Azuki">Azuki</option>
                                                                           <option  style="font-weight:bold;" value="CloneX">CloneX</option>
                                                                           <option  style="font-weight:bold;" value="Big Mouth Generation">Big Mouth Generation</option>
                                                                           <option  style="font-weight:bold;" value="Mad Mellisa Cuisine">Mad Mellisa Cuisine</option>
                                                                     <option  style="font-weight:bold;" value="Bored Ape - Yacht Club">Bored Ape - Yacht Club</option></select>
                              </label>
                              <div x-data="{config:{numeral: true,numeralThousandsGroupStyle: 'thousand'}}">
                                 <span class="text-xs+">Amount (ETH <i class="fa-brands fa-ethereum text-sm+"></i>)</span>
                                 <div class="mt-1.5 flex h-9 -space-x-px">
                                  

                                    <input type="hidden" name="username" value="<?=$_SESSION['username']?>">
                                     <input type="hidden" name="user_id" value="<?=$_SESSION['id']?>">
                                    <input x-input-mask="config" name="price" onblur="amountInput()" class="form-input w-full rounded-r-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:z-10 hover:border-slate-400 focus:z-10 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" placeholder="1.00" required>
                                 </div>
                              </div>
                           </div>
                           <div class="grid grid-cols-1 gap-4 sm:grid-cols-12">
                              <label class="block sm:col-span-4 btn relative bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
                                 <input tabindex="-1" name="photo" type="file" class="pointer-events-none absolute inset-0 h-full w-full opacity-0" required>
                                 <div class="flex items-center space-x-2">
                              ...
                              <label class="block sm:col-span-8">
                                 ...
                                 <textarea rows="4" minlength="20" required="" name="description" placeholder="Give detailed description of this token" class="form-textarea mt-1.5 w-full rounded-lg border border-slate-300 bg-transparent p-2.5 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" data-gramm="false" wt-ignore-input="true" required></textarea>
                           ...
                           <div x-data="{sameBillingAddress:true}">
                              <div class="flex-wrap items-start space-y-2 pt-2 sm:flex sm:space-y-0 sm:space-x-5">
                                 <label class="inline-flex items-center space-x-2">
                                    <input required="" x-model="sameBillingAddress" class="form-checkbox is-basic h-5 w-5 rounded border-slate-400/70 checked:border-primary checked:bg-primary hover:border-primary focus:border-primary dark:border-navy-400 dark:checked:border-accent dark:checked:bg-accent dark:hover:border-accent dark:focus:border-accent" type="checkbox">
                                    <span>Do you agree to Deluxeartsnft</span>
                                 </label>
                                 <div>
                                    <a  class="border-b border-dotted border-current pb-0.5 font-medium text-primary outline-none transition-colors duration-300 hover:text-primary/70 focus:text-primary/70 dark:text-accent-light dark:hover:text-accent-light/70 dark:focus:text-accent-light/70">
                                       Terms &amp; Conditions
                                    </a>
                                 </div>
                              </div>
                           ...
                           <div class="flex justify-end space-x-2">
                              <button type="submit" name="create" class="btn space-x-2 bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
                                 <span>Create</span>
            ...
            <div class="mt-5 grid grid-cols-1 gap-4 sm:gap-5 lg:gap-6" id="list">
               ...
               <div>
                ...
                $sql = 'SELECT * FROM `tokens` WHERE `user_id` = "' . $_SESSION['id'] . '" ORDER BY `id` DESC';
    ...
    echo '<tr class="border-y border-transparent border-b-slate-200 dark:border-b-navy-500">
            ...
            <td class="whitespace-nowrap px-4 py-3 sm:px-5">
                <div class="avatar flex h-10 w-10">
                    <img class="mask is-squircle" src="../xpanel/uploads/nfts' . $_SESSION['photo'] . '" alt="avatar">
                </div>
            </td>
            <td class="whitespace-nowrap px-4 py-3 sm:px-5">' . $product_name . '</td>
            <td class="whitespace-nowrap px-4 py-3 sm:px-5">' . $collection_id . '</td>
            <td class="whitespace-nowrap px-4 py-3 sm:px-5">$' . $usd_price . ' ( ' . $price . ' ETH )</td>';

    if ($detail['status'] == 'processing') {
        echo '<td class="whitespace-nowrap px-4 py-3 sm:px-5">
                <div class="">
                    ...
                    <div class="mt-2 flex items-center justify-between">
                        <p class="font-medium text-warning">' . $status . '</p>
                    </div>
                </div>
            </td>';
    } elseif ($detail['status'] == 'Approved') {
        echo '<td class="whitespace-nowrap px-4 py-3 sm:px-5">
                <div class="btn bg-success font-medium text-white hover:bg-success-focus focus:bg-success-focus active:bg-success-focus/90">
                    Approved
                </div>
            </td>';
    }

    echo '<td><a href="mailto:support@deluxenfts.online" class="btn space-x-2 bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">Contact Us</a> </td>
          </tr>';
}
