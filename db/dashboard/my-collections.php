...
<div class="mt-3 grid grid-cols-12 gap-4 sm:gap-5 lg:gap-6">
            ...
            <div class="card p-4 sm:p-5">
              ...
              <ul class="mt-6 space-y-1.5 font-inter font-medium">
                <a class="flex items-center space-x-2 rounded-lg bg-primary px-4 py-2.5 tracking-wide text-white outline-none transition-all dark:bg-accent" href="#">
                  <i class="fa-solid fa-camera"></i>
                  <span>Your Purchased Tokens</span>
                </a>
                <a class="flex items-center space-x-2 rounded-lg bg-success px-4 py-2.5 tracking-wide text-white outline-none transition-all dark:bg-success" href="#">
                  <i class="fa-solid fa-money"></i>
                  <span>Make Quick Sale</span>
                </a>
              </ul>
            </div>
          </div>
         ...
         <?php
// Query to fetch approved NFTs for the current session user from the 'buy' table
$sql8 = 'SELECT * FROM `buy` WHERE `account_id` = "' . $_SESSION['account_id'] . '" AND `status` = "Approved" ORDER BY `id` DESC';
$run_sql8 = $connect->query($sql8);
$row8 = $run_sql8->num_rows;
...
if ($row8 < 1) {
                    ...
                    <div class="col-12">
                        <div class="card">
                            <div class="card-header">
                                <h3 class="card-title">No NFT Purchased Yet!</h3>
                            </div>
    ...
    // Approved NFTs found, display them
    echo '<div class="mt-4 grid grid-cols-1 gap-4 sm:mt-5 sm:grid-cols-3 sm:gap-5 lg:mt-6 lg:gap-6">';
    while ($detail8 = $run_sql8->fetch_assoc()) {
        $nft_picture = $detail8['nft_picture'];
        $author_name = $detail8['author_name'];
        $nft_amount = $detail8['nft_amount'];
        $status = $detail8['status'];

        echo '
            <div class="card p-2 pb-3">
                <div class="relative w-full">
                    <img class="h-56 w-full rounded-xl object-cover object-center" src="' . $nft_picture . '" alt="NFT Image">
            ...
            <div class="card p-2 pb-3">
                ...
                <div class="mx-2 mt-3">
                    ...
                    <div class="mt-2 flex items-center justify-between">
                        ...
                        <div class="flex">
                            <div x-data="{showModal:true}">
                                <a  class="btn space-x-2 rounded-full bg-success font-medium text-white hover:bg-success-focus focus:bg-success-focus active:bg-success-focus/90 dark:bg-success dark:hover:bg-success-focus dark:focus:bg-success-focus dark:active:bg-success/90">
                                    <i class="fa-solid fa-money text-sm+"></i>
                                    <span>Approved</span>
                                </a>
                            </div>
...
<?php
                ...
                $sql = 'SELECT * FROM `tokens` WHERE `user_id` = "' . $_SESSION['id'] . '" AND `status` = "Approved" ORDER BY `id` DESC';
...
while ($detail = $run_sql->fetch_assoc()) {
    ...
    echo '<td><a href="mailto:info@deluxeartsnft.online" class="btn space-x-2 bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">Contact Us</a> </td>
          </tr>';
}
